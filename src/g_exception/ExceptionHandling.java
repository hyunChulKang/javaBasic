...
public class ExceptionHandling {
	...
	public static void main(String[] args) {
			 ...
			 * -예외 : 프로그램에 의해서 수집될 수 있는 다소 미약한 오류(처리 가능)
			 * 
			 * <<예외>>
			 * -모든예외는Excepion 클래스의 자식 클래스이다.
			 ...
			 * 
			 * <<예외처리(try~ catch)>>
			 * -예외처리를 통해 프로그램이 비정상적으로  종료되는 것을 방지 할수있다.
			 * -try{} catch(Exception e) {}
			 * -try 블럭 안의 내용을 실행 중 예외가 발생하면 catch로 넘어간다.
			 * -catch의 ()안에는 처리할 예외를 지정해  줄 수있다.
			 * -여러 종류의 예외를 처리할 수 있도록 catch는 하나 이상 올 수 있다.
			 * -발생한 예외와 일치하는 catch 블러안의 내용이 수행된 후 try~ catch를 빠져나간다.
			 * -발생한 예외와 일치하는 catch가 없을 경우 예외는 처리가 되지않는다.
			 */
			try{
